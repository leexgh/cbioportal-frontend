...
import { VariantAnnotation, GenomeNexusAPI } from 'cbioportal-frontend-commons';
...
export function fetch(queries: Mutation[]): Promise<VariantAnnotation[]> {
    if (queries.length > 0) {
        return fetchVariantAnnotationsByMutation(
            ...
            new GenomeNexusAPI(AppConfig.serverConfig.genomenexus_url_grch38!)
        );
    } else {
...
export default class GenomeNexusMutationAssessorGrch38Cache extends LazyMobXCache<
    VariantAnnotation,
    Mutation
> {
    constructor() {
